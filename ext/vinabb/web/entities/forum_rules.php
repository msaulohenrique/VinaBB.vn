...
/**
* Entity for a single forum rules
*/
class forum_rules extends forum implements forum_rules_interface
{
	/**
	...
	*/
	public function get_rules_for_edit()
	{
		...
		$text = isset($this->data['forum_rules']) ? $this->data['forum_rules'] : '';
		$uid = isset($this->data['forum_rules_uid']) ? $this->data['forum_rules_uid'] : '';
		$options = isset($this->data['forum_rules_options']) ? (int) $this->data['forum_rules_options'] : 0;
	...
	*/
	public function get_rules_for_display($censor = true)
	{
		...
		$text = isset($this->data['forum_rules']) ? $this->data['forum_rules'] : '';
		$uid = isset($this->data['forum_rules_uid']) ? $this->data['forum_rules_uid'] : '';
		$bitfield = isset($this->data['forum_rules_bitfield']) ? $this->data['forum_rules_bitfield'] : '';
		$options = isset($this->data['forum_rules_options']) ? (int) $this->data['forum_rules_options'] : 0;
	...
	*/
	public function set_rules($text)
	{
		...
		$this->data['forum_rules_uid'] = $uid;
		$this->data['forum_rules_bitfield'] = $bitfield;
	...
	*/
	...
	{
		return (bool) ($this->data['forum_rules_options'] & OPTION_FLAG_BBCODE);
	}
	...
	*/
	...
	{
		$this->set_rules_options(OPTION_FLAG_BBCODE, !$enable);
	...
	*/
	...
	{
		return (bool) ($this->data['forum_rules_options'] & OPTION_FLAG_LINKS);
	}
	...
	*/
	...
	{
		$this->set_rules_options(OPTION_FLAG_LINKS, !$enable);
	...
	*/
	...
	{
		return (bool) ($this->data['forum_rules_options'] & OPTION_FLAG_SMILIES);
	}
	...
	*/
	...
	{
		$this->set_rules_options(OPTION_FLAG_SMILIES, !$enable);
	...
	*/
	protected function set_rules_options($value, $negate = false, $reparse_content = true)
	{
		// Set article_text_options to 0 if it does not yet exist
		$this->data['forum_rules_options'] = isset($this->data['forum_rules_options']) ? $this->data['forum_rules_options'] : 0;

		// If we're setting the option and the option is not already set
		if (!$negate && !($this->data['forum_rules_options'] & $value))
		{
			// Add the option to the options
			$this->data['forum_rules_options'] += $value;
		}
		...
		if ($reparse_content && $this->data['forum_rules'] != '')
		{
			...
			decode_message($text, $this->data['forum_rules_uid']);
