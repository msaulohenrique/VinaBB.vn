...
function get_folder($user_id, $folder_id = false)
{
	...
	foreach ($folder as $f_id => $folder_ary)
	{
		...
		$template->assign_block_vars('folder', array(
			...
			'U_FOLDER'			=> $controller_helper->route('vinabb_web_ucp_route', array('id' => 'pm', 'folder' => ($f_id > 0) ? $f_id : $folder_id_name)),
		...
		!in_array($cur_folder_id, array(PRIVMSGS_NO_BOX, PRIVMSGS_OUTBOX)) && $cur_folder_id != $dest_folder)
	{
		...
		if ($dest_folder != PRIVMSGS_INBOX)
		{
			...
			if ($message_limit && $row['pm_count'] + sizeof($move_msg_ids) > $message_limit)
			{
				...
				$message .= sprintf($user->lang['CLICK_RETURN_FOLDER'], '<a href="' . $controller_helper->route('vinabb_web_ucp_route', array('id' => 'pm', 'folder' => $row['folder_id'])) . '">', '</a>', $row['folder_name']);
		...
		else
		{
			...
			if ($message_limit && $num_messages + sizeof($move_msg_ids) > $message_limit)
			{
				...
				$message .= sprintf($user->lang['CLICK_RETURN_FOLDER'], '<a href="' . $controller_helper->route('vinabb_web_ucp_route', array('id' => 'pm', 'folder' => 'inbox')) . '">', '</a>', $user->lang['PM_INBOX']);
				trigger_error($message);
			}
		}
	...
	switch ($mark_action)
	{
		...
		case 'delete_marked':
			...
			if (confirm_box(true))
			{
				...
				$redirect = $controller_helper->route('vinabb_web_ucp_route', array('id' => 'pm', 'folder' => $cur_folder_id));
		...
		if (sizeof($address) && !$plaintext)
		{
			...
			foreach ($address as $type => $adr_ary)
			{
				foreach ($adr_ary as $id => $row)
				{
					...
					{
						$tpl_ary = array_merge($tpl_ary, array(
							'U_VIEW'		=> $controller_helper->route('vinabb_web_user_group_route', array('id' => $id)),
						));
					}
...
function message_history($msg_id, $user_id, $message_row, $folder, $in_post_mode = false)
{
	...
	do
	{
		...
		$row['folder'][] = (isset($folder[$folder_id])) ? '<a href="' . $controller_helper->route('vinabb_web_user_group_route', array('id' => 'pm', 'folder' => $folder_id)) . '">' . $folder[$folder_id]['folder_name'] . '</a>' : $user->lang['UNKNOWN_FOLDER'];

		if (isset($rowset[$row['msg_id']]))
		{
			$rowset[$row['msg_id']]['folder'][] = (isset($folder[$folder_id])) ? '<a href="' . $controller_helper->route('vinabb_web_user_group_route', array('id' => 'pm', 'folder' => $folder_id)) . '">' . $folder[$folder_id]['folder_name'] . '</a>' : $user->lang['UNKNOWN_FOLDER'];
		}
		else
		{
	...
	for ($i = 0, $size = sizeof($rowset); $i < $size; $i++)
	{
		...
		$template->assign_block_vars('history_row', array(
			...
			'USER_ID'			=> $row['user_id'],
			'U_VIEW_MESSAGE'	=> $controller_helper->route('vinabb_web_ucp_route', array('id' => 'pm', 'f' => $folder_id, 'p=' => $row['msg_id'])),
			'U_QUOTE'			=> (!$in_post_mode && $auth->acl_get('u_sendpm') && $author_id != ANONYMOUS) ? $controller_helper->route('vinabb_web_ucp_route', array('id' => 'pm', 'mode' => 'compose', 'action' => 'quote', 'f'=> $folder_id, 'p' => $row['msg_id'])) : '',
			'U_POST_REPLY_PM'	=> ($author_id != $user->data['user_id'] && $author_id != ANONYMOUS && $auth->acl_get('u_sendpm')) ? $controller_helper->route('vinabb_web_ucp_route', array('id' => 'pm', 'mode' => 'compose', 'action' => 'reply', 'f' => $folder_id, 'p' => $row['msg_id'])) : '')
		);
		unset($rowset[$i]);
		$prev_id = $id;
	}
	...
	$template->assign_vars(array(
		...
		'U_VIEW_NEXT_HISTORY'		=> ($next_history_pm) ? $controller_helper->route('vinabb_web_ucp_route', array('id' => 'pm', 'p' => $next_history_pm)) : '',
		'U_VIEW_PREVIOUS_HISTORY'	=> ($previous_history_pm) ? $controller_helper->route('vinabb_web_ucp_route', array('id' => 'pm', 'p' => $previous_history_pm)) : '',
	));
		...
		foreach ($adr_ary as $type => $id_ary)
		{
			foreach ($id_ary as $ug_id => $_id)
			{
				...
				else
				{
					...
					$link = '<a href="' . $controller_helper->route('vinabb_web_user_group_route', array('id' => $ug_id)) . '"' . $user_colour . '>';
