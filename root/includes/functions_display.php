	...
	foreach ($forum_rows as $row)
	{
		...
		if ($row['parent_id'] == $root_data['forum_id'] && $row['forum_type'] == FORUM_CAT)
		{
			...
			$cat_row = array(
				...
				'U_VIEWFORUM'			=> $controller_helper->route('vinabb_web_board_forum_route', array('forum_id' => $row['forum_id'])),
			);
		...
		if (isset($subforums[$forum_id]))
		{
			foreach ($subforums[$forum_id] as $subforum_id => $subforum_row)
			{
				...
				{
					$subforums_list[] = array(
						'link'		=> $controller_helper->route('vinabb_web_board_forum_route', array('forum_id' => $subforum_id)),
		...
		if ($row['forum_last_post_id'])
		{
			...
			$last_post_url = $controller_helper->route('vinabb_web_board_post_route', array('post_id' => $row['forum_last_post_id']));
		}
		else
		{
		...
		$forum_row = array(
			...
			'U_UNAPPROVED_TOPICS'	=> ($row['forum_id_unapproved_topics']) ? $controller_helper->route('vinabb_web_mcp_route', array('id' => 'queue', 'mode' => 'unapproved_topics', 'f' => $row['forum_id_unapproved_topics'])) : '',
			'U_UNAPPROVED_POSTS'	=> ($row['forum_id_unapproved_posts']) ? $controller_helper->route('vinabb_web_mcp_route', array('id' => 'queue', 'mode' => 'unapproved_posts', 'f' => $row['forum_id_unapproved_posts'])) : '',
...
function generate_forum_nav(&$forum_data_ary)
{
	...
	if (!empty($forum_parents))
	{
		foreach ($forum_parents as $parent_forum_id => $parent_data)
		{
			...
			$navlinks_parents[] = array(
				...
				'U_VIEW_FORUM'	=> $controller_helper->route('vinabb_web_board_forum_route', array('forum_id' => $parent_forum_id)),
			);
		}
	}
	...
	$navlinks = array(
		...
		'U_VIEW_FORUM'	=> $controller_helper->route('vinabb_web_board_forum_route', array('forum_id' => $forum_data_ary['forum_id'])),
	);
...
function display_user_activity(&$userdata_ary)
{
	...
	*/
	...
	$template->assign_vars(array(
		...
		'U_ACTIVE_FORUM'		=> $controller_helper->route('vinabb_web_board_forum_route', array('forum_id' => $active_f_id)),
		'U_ACTIVE_TOPIC'		=> $controller_helper->route('vinabb_web_board_topic_route', array('topic_id' => $active_t_id)),
...
function watch_topic_forum($mode, &$s_watching, $user_id, $forum_id, $topic_id, $notify_status = 'unset', $start = 0, $item_title = '')
{
	...
	$route_name = ($mode == 'forum') ? 'vinabb_web_board_forum_route' : 'vinabb_web_board_topic_route';
	$match_id_field = ($mode == 'forum') ? 'forum_id' : 'topic_id';
		...
		if (!is_null($notify_status) && $notify_status !== '')
		{
			if (isset($_GET['unwatch']))
			{
				...
				if ($token && check_link_hash($token, "{$mode}_$match_id") || confirm_box(true))
				{
					...
					{
						$redirect_url = $controller_helper->route($route_name, array($match_id_field => $match_id, 'uid' => $user->data['user_id'], 'start' => $start));
	...
	if ($can_watch)
	{
		$s_watching['link'] = $controller_helper->route($route_name, array($match_id_field => $match_id, 'uid' => $user->data['user_id'], (($is_watching) ? 'unwatch' : 'watch') => $mode, 'start' => $start, 'hash' => generate_link_hash("{$mode}_$match_id")));
		$s_watching['link_toggle'] = $controller_helper->route($route_name, array($match_id_field => $match_id, 'uid' => $user->data['user_id'], ((!$is_watching) ? 'unwatch' : 'watch') => $mode, 'start' => $start, 'hash' => generate_link_hash("{$mode}_$match_id")));
...
function phpbb_show_profile($data, $user_notes_enabled = false, $warn_user_enabled = false, $check_can_receive_pm = true)
{
	...
	$template_data = array(
		...
		'U_SEARCH_USER' => ($auth->acl_get('u_search')) ? $controller_helper->route('vinabb_web_search_route', array('author_id' => $user_id, 'sr' => 'posts')) : '',
		'U_NOTES'		=> ($user_notes_enabled && $auth->acl_getf_global('m_')) ? $controller_helper->route('vinabb_web_mcp_route', array('id' => 'notes', 'mode' => 'user_notes', 'u' => $user_id), true, $user->session_id) : '',
