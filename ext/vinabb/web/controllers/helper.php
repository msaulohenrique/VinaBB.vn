...
class helper implements helper_interface
{
	...
	*/
	public function build_icon_list($selected_icon)
	{
		...
		{
			$icon_options .= '<optgroup label="' . $type_data['name'] . '">"';

			foreach ($type_data['data'] as $icon)
			{
				$icon_value = $type_data['prefix'] . $icon;
				$icon_name = ucwords(str_replace($type_data['char'], ' ', $icon));
