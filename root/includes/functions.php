...
function login_box($redirect = '', $l_explain = '', $l_success = '', $admin = false, $s_display = true)
{
		...
		switch ($result['status'])
		{
			case LOGIN_ERROR_PASSWORD_CONVERT:
				$err = sprintf(
					...
					($config['email_enable']) ? '<a href="' . $controller_helper->route('vinabb_web_ucp_route', array('id' => 'front', 'mode' => 'sendpassword')) . '">' : '',
	...
	$template->assign_vars(array(
		...
		'U_SEND_PASSWORD' 		=> ($config['email_enable']) ? $controller_helper->route('vinabb_web_ucp_route', array('id' => 'front', 'mode' => 'sendpassword')) : '',
		'U_RESEND_ACTIVATION'	=> ($config['require_activation'] == USER_ACTIVATION_SELF && $config['email_enable']) ? $controller_helper->route('vinabb_web_ucp_route', array('id' => 'front', 'mode' => 'resend_act')) : '',
...
function page_header($page_title = '', $display_online_list = false, $item_id = 0, $item = 'forum', $send_headers = true)
{
	...
	*/
	...
	{
		$u_login_logout = $controller_helper->route('vinabb_web_ucp_route', array('id' => 'front', 'mode' => 'logout'), true, $user->session_id);
		$l_login_logout = $user->lang['LOGOUT'];
	}
	else
	{
		$u_login_logout = $controller_helper->route('vinabb_web_ucp_route', array('id' => 'front', 'mode' => 'login'));
	...
	$template->assign_vars(array(
		...
		'U_VIEW_ALL_NOTIFICATIONS'		=> $controller_helper->route('vinabb_web_ucp_route', array('id' => 'notifications')),
		'U_MARK_ALL_NOTIFICATIONS'		=> $controller_helper->route('vinabb_web_ucp_route', array('id' => 'notifications', 'mode' => 'notification_list', 'mark' => 'all', 'token' => $notification_mark_hash)),
		'U_NOTIFICATION_SETTINGS'		=> $controller_helper->route('vinabb_web_ucp_route', array('id' => 'notifications', 'mode' => 'notification_options')),
		...
		'U_SEARCH'				=> $controller_helper->route('vinabb_web_search_route'),
		'U_SITE_HOME'			=> $config['site_home_url'],
		'U_REGISTER'			=> $controller_helper->route('vinabb_web_ucp_route', array('id' => 'front', 'mode' => 'register')),
		'U_PROFILE'				=> $controller_helper->route('vinabb_web_ucp_default_route'),
		...
		'U_SEARCH_UNANSWERED'	=> $controller_helper->route('vinabb_web_search_route', array('mode' => 'unanswered')),
		...
		'S_LOGIN_ACTION'		=> ((!defined('ADMIN_START')) ? $controller_helper->route('vinabb_web_ucp_route', array('id' => 'front', 'mode' => 'login')) : append_sid("{$phpbb_admin_path}index.$phpEx", false, true, $user->session_id)),
...
function phpbb_get_board_contact(\phpbb\config\config $config, $phpEx)
{
	...
	{
		return generate_board_url(true) . $controller_helper->route('vinabb_web_ucp_route', array('mode' => 'contactadmin'));
	}
	else
	{
...
function phpbb_get_board_contact_link(\phpbb\config\config $config, $phpbb_root_path, $phpEx)
{
	...
	{
		return $controller_helper->route('vinabb_web_ucp_route', array('mode' => 'contactadmin'));
	}
	else
	{
