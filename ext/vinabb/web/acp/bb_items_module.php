...
class bb_items_module
{
	...
	public function main($id, $mode)
	{
		...
		switch ($action)
		{
			...
			case 'save':
				...
				{
					trigger_error($this->language->lang('ERROR_BB_CAT_NAME_EMPTY') . adm_back_link($this->u_action), E_USER_WARNING);
				}

				if (empty($cat_varname))
				{
					trigger_error($this->language->lang('ERROR_BB_CAT_VARNAME_EMPTY') . adm_back_link($this->u_action), E_USER_WARNING);
				}
				else
				{
					$sql = 'SELECT *
						FROM ' . $this->bb_categories_table . '
						WHERE bb_tyle = ' . constants::BB_TYPE_ . strtoupper($mode) . "
					...
					if (sizeof($rows))
					{
						trigger_error($this->language->lang('ERROR_BB_CAT_VARNAME_DUPLICATE', $cat_varname) . adm_back_link($this->u_action), E_USER_WARNING);
					}
				}

				$sql_ary = array(
					'bb_type'		=> constants::BB_TYPE_ . strtoupper($mode),
					'cat_name'		=> $cat_name,
		...
		$cats = array();
		$cat_count = 0;
		$start = $this->list_bb_cats($this->bb_categories_table, $mode, $cats, $cat_count, $per_page, $start);
	...
	* @return int
	*/
	private function list_bb_cats(&$bb_categories_table, &$mode, &$cats, &$cat_count, $limit = 0, $offset = 0)
	{
		$sql = "SELECT COUNT(cat_id) AS cat_count
			FROM $bb_categories_table
			WHERE bb_type = " . constants::BB_TYPE_ . strtoupper($mode);
		$result = $this->db->sql_query($sql);
		$cat_count = (int) $this->db->sql_fetchfield('style_count');
		$this->db->sql_freeresult($result);
